            ...
            var payload = new { 
                embeds = new[] { 
                    new { 
                        ...
                        color = color,
                        footer = new { text = "Lan JDL System - " + DateTime.Now.ToString("HH:mm") }
                    } 
                } 
            };
            
            webrequests.Enqueue(url, JsonConvert.SerializeObject(payload), (code, response) => {}, this, RequestMethod.POST, new Dictionary<string, string> { { "Content-Type", "application/json" } });
        }
